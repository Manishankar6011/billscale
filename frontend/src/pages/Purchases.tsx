...
import { Plus, ShoppingCart, Trash2, Loader2, Search, ArrowLeft } from 'lucide-react';
...
const Purchases = () => {
    ...
    const { data: purchases = [], isLoading: purchasesLoading } = useQuery<Purchase[]>({
        queryKey: ['purchases'],
        queryFn: async () => {
            const res = await axios.get<Purchase[]>('/api/transactions/purchases', { headers: { Authorization: `Bearer ${user?.token}` } });
            return res.data;
        },
        enabled: !!user?.token
    });
    ...
    const { data: products = [], isLoading: productsLoading } = useQuery<Product[]>({
        queryKey: ['inventory'],
        queryFn: async () => {
            const res = await axios.get<Product[]>('/api/inventory', { headers: { Authorization: `Bearer ${user?.token}` } });
            return res.data;
        },
        enabled: !!user?.token
    });
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {products
                                        .filter(p => p.name.toLowerCase().includes(itemSearch.toLowerCase()) || (p.barcode && p.barcode.includes(itemSearch)))
                                        .map((p) => (
